...
def rename_file_contents(
    root_dir_path: str, old_name: str, new_name: str, exclude_dirs: list = None
):
...
if __name__ == "__main__":
    ...
    old_name = "memorax-template"
    new_name = sys.argv[1]

    print(
        f"Warning, this script will rename all instances of '{old_name}' to '{new_name}' in {root_dir_path}."
    )
    proceed = input("Proceed? (y/n): ")

    if proceed.lower() == "y":
        rename_file_contents(
            root_dir_path, old_name, new_name, exclude_dirs=[".git", "logs"]
        )
    else:
        print("Aborting.")
